...
def get_proxy_list():
    ...
    # Читаем СТРОГО csv.csv. Никаких запасных вариантов!
    if os.path.exists("csv.csv"):
        ...
        print("\n⚠ ФАЙЛ csv.csv НЕ НАЙДЕН НА СЕРВЕРЕ! Загрузите его на GitHub в корень репозитория!", flush=True)
...
def start_parser_internal():
    ...
    if not proxies:
        print("\n❌ КРИТИЧЕСКАЯ ОШИБКА: Прокси не загружены! Парсер остановлен.", flush=True)
        return False
        ...
        try:
            if parser_process and parser_process.stdout:
                ...
                if line:
                    ...
                    # Триггеры для моментальной смены прокси
                    ...
                    if any(err in line for err in triggers):
                        print("\n[АВТОПИЛОТ] Бан от Авито! Берем СЛЕДУЮЩИЙ прокси из 1000...\n", flush=True)
                        with parser_lock:
